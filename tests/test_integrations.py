...
MOCK_API_SUCCESS_DATA = {"taxId": "11223344000155", "company": {"name": "Empresa de Teste Sucesso"}}
...
def create_mock_response(status_code: int, json_data: dict = None) -> httpx.Response:
    ...
    response = httpx.Response(status_code=status_code, json=json_data, request=httpx.Request("GET", ""))
...
async def test_cnpj_provider_uses_primary_key_first(mock_httpx_client, monkeypatch):
    ...
    mock_httpx_client.get.return_value = create_mock_response(200, MOCK_API_SUCCESS_DATA)

    provider = CNPJaProvider()
    result = await provider.get_cnpj_data("11223344000155")

    assert result == MOCK_API_SUCCESS_DATA
    called_headers = mock_httpx_client.get.call_args[1]['headers']
    assert called_headers['Authorization'] == 'Bearer valid_primary_key'

@pytest.mark.asyncio
async def test_cnpj_provider_falls_back_to_secondary_key(mock_httpx_client, monkeypatch):
