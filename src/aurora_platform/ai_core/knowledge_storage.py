...
class KnowledgeEntry(BaseModel):
    ...
    source_id: str = Field(
        ..., description="Identificador único da fonte do conhecimento"
    )
...
class VectorKnowledgeBase:
    ...
    def add_entry(self, entry: KnowledgeEntry):
        ...
        if len(entry.embedding) != EMBEDDING_DIM:
            raise ValueError(
                f"Dimensão do embedding inválida. Esperado {EMBEDDING_DIM}, recebido {len(entry.embedding)}"
            )
    ...
    def _create_mock_entry(self) -> KnowledgeEntry:
        ...
        return KnowledgeEntry(
            content=content, embedding=embedding, source_id="mock_doc_001"
        )
