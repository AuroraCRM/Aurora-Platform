...
class SecurityUtils:
    ...
    async def validate_request_security(request: Request) -> None:
        ...
        for param, values in request.query_params.items():
            ...
            if SecurityUtils.detect_sql_injection(param) or SecurityUtils.detect_sql_injection(values):
                logger.warning(f"Possível SQL Injection detectado em parâmetro: {param}={values}")
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Parâmetro de consulta inválido"
                )