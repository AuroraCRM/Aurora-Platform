...
class ServicoCRM:
    ...
    async def create_cliente_from_cnpj(self, cnpj: str) -> Dict[str, Any]:
        ...
        if self.cliente_repo.get_by_cnpj(cnpj=cnpj_limpo):
            raise HTTPException(status_code=400, detail=f"Cliente com CNPJ {cnpj_limpo} já cadastrado.")
        ...
        if cached_data:
            ...
            try:
                ...
                logger.error(f"Erro inesperado ao consultar CNPJ {cnpj_limpo}: {str(e)}")
        ...
        try:
            ...
            raise HTTPException(status_code=422, detail="Erro ao processar dados do CNPJ")
    
    def _map_cnpj_data_to_cliente(self, data: Dict[str, Any], cnpj_limpo: str) -> ClienteCreate:
        ...
        if "company" in data:
            ...
            email = emails[0].get("address", "contato@exemplo.com") if emails else "contato@exemplo.com"
            telefone = ""
            if phones:
...
def listar_todos_os_clientes(db: Session, skip: int = 0, limit: int = 100) -> List[ClienteDB]:
    """Lista todos os clientes com paginação."""
    return db.query(ClienteDB).order_by(ClienteDB.razao_social).offset(skip).limit(limit).all()

def atualizar_cliente(db: Session, cliente_id: int, cliente_update: ClienteUpdate) -> Optional[ClienteDB]:
...
async def call_cnpja_api(base_url: str, cnpj: str, api_key: str = None) -> Dict[str, Any]:
    """
