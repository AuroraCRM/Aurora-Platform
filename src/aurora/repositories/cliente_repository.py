...
class ClienteRepository:
    ...
    def search_by_name(self, search_term: str, skip: int = 0, limit: int = 100) -> List[ClienteDB]:
        """
        ...
        ).order_by(ClienteDB.razao_social).offset(skip).limit(limit).all()
    
    def filter_by_segment(self, segment: str, skip: int = 0, limit: int = 100) -> List[ClienteDB]:
        """
        ...
        """
        return self.db.query(ClienteDB).filter(
            ClienteDB.segmento == segment
        ).order_by(ClienteDB.razao_social).offset(skip).limit(limit).all()
    
    def get_by_date_range(self, start_date: datetime, end_date: datetime, skip: int = 0, limit: int = 100) -> List[ClienteDB]:
        """
        ...
        ).order_by(ClienteDB.data_criacao.desc()).offset(skip).limit(limit).all()
    ...
    def get_paginated(self, skip: int = 0, limit: int = 100) -> Tuple[List[ClienteDB], int]:
        """
