...
class KnowledgeEntry(BaseModel):
    ...
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Metadados da entrada")
    source_id: Optional[str] = Field(None, description="ID da fonte de dados")
    timestamp: datetime = Field(default_factory=datetime.now, description="Data e hora de criação")
...
class KnowledgeStorage:
    ...
    def store_embedding(self, data: str, embedding: List[float], metadata: Dict[str, Any]) -> str:
        """
    ...
    def retrieve_similar(self, query_embedding: List[float], top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        return list(self._memory_storage.values())[:min(top_k, len(self._memory_storage))]
    
    def store_structured_knowledge(self, entity_type: str, entity_data: Dict[str, Any]) -> str:
        """
