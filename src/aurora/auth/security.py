...
from aurora.repositories.cliente_repository import (
    ClienteRepository,
)  # Assumindo que o usuário é um 'Cliente'
...
def get_current_user(
    ...
    user = ClienteRepository(db).get_by_email(
        email=username
    )  # Busca o usuário pelo email (que é o 'sub')
    if user is None:
        raise credentials_exception
    return user
