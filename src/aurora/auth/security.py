...
async def get_current_user(
    ...
    try:
        ...
        token_data = TokenData(
            username=username, scopes=payload.get("scopes", []), jti=jti
        )
        ...
        logger.info(
            f"Acesso autenticado: {username} de {client_ip} usando {user_agent}"
        )
...
async def authenticate_user(username: str, password: str) -> Dict[str, Any]:
    ...
    if failed_attempts[username]["count"] >= 5:
        ...
        logger.warning(
            f"Conta bloqueada após múltiplas tentativas de login: {username}"
        )
