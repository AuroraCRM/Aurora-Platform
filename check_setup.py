...
def check_python_version():
    ...
    if current_version < required_version:
        logger.error(f"Versão do Python incompatível. Requer Python {required_version[0]}.{required_version[1]} ou superior.")
        return False
...
def main():
    ...
    if python_ok and deps_ok and env_ok:
        ...
        db_ok = False
        logger.warning("Pulando verificação de banco de dados devido a erros anteriores")
    ...
    if all([python_ok, deps_ok, env_ok, db_ok]):
        ...
        logger.error("❌ Ambiente com problemas de configuração. Corrija os erros acima.")
        return 1
