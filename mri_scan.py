...
def generate_report():
    ...
    report_content.append(
        f"Análise profunda do diretório: `{os.path.abspath(TARGET_DIRECTORY)}`\n\n---"
    )
    ...
    for key, tool_config in TOOLS.items():
        ...
        if returncode == -1:
            report_content.append(
                "Falha ao executar a ferramenta. Verifique se ela está instalada.\n"
            )
            continue
        ...
        if stdout or stderr:
            ...
            report_content.append(
                "✅ Nenhuma questão encontrada por esta ferramenta.\n"
            )
